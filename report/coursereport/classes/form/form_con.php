...
class form_con extends \moodleform {
    ...
    public function definition() {
        ...
        $PAGE->requires->js('/report/coursereport/js/controller_formJS.js', false);

        // Desactivar el chequeo de cambios del formulario
        $mform->disable_form_change_checker();

        $list_of_customers=$DB->get_records('customer',[],'','id,shortname');
        foreach ($list_of_customers as $key=>$customer){
            $list_of_customers[$key]=$customer->shortname;
        }
        $mform->addElement('select', 'selCustomer', get_string('selCustomer', 'block_itp'), $list_of_customers, '');

        //Se obtiene el valor del campo personalizado customer
        $customerid=array_key_first($list_of_customers);

        //Se obtiene el valor del campo personalizado customer shortname
        $customer_shortname=$DB->get_field('customer', 'shortname', array('id'=>$customerid), IGNORE_MISSING);
