...
class get_list_addresses_excel extends \core_external\external_api {
    ...
    public static function execute($params) {
        ...
        foreach ($trainee_query as $user) {
            $addresses=$DB->get_records('ticketmanagement_address',['userid'=>$user->id]);
            $address_list=[];
            foreach ($addresses as $address){
                $address_list[]=[
                    ...
                    'town'=>$address?->town ?? '',
                    'entry_date'=>$address?->entry_date ?? 0,
                    'departure_date'=>$address?->departure_date ?? 0,
                ];
            };
            $total_address_list = array_merge($total_address_list, $address_list);
        }
        
        $users=[
            'address_list'=>$total_address_list,
    ...
    public static function execute_returns() {
        ...
        return new external_single_structure(
            array(
                'address_list' => new external_multiple_structure(
                    new external_single_structure([
                        ...
                        'type'=> new external_value(PARAM_TEXT, 'brand'),
                        'address'=> new external_value(PARAM_TEXT, 'model'),
                        'house'=> new external_value(PARAM_INT, 'color'),
                        'floor'=> new external_value(PARAM_INT, 'color'),
                        'block'=> new external_value(PARAM_TEXT, 'Plate number'),
                        'door'=> new external_value(PARAM_TEXT, 'Plate number'),
                        'number'=> new external_value(PARAM_INT, 'Plate number'),
                        'town'=> new external_value(PARAM_TEXT, 'Plate number'),
                        'entry_date'=> new external_value(PARAM_INT, 'date '),
                        'departure_date'=> new external_value(PARAM_INT, 'date '),
