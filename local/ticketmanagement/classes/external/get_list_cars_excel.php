...
class get_list_cars_excel extends \core_external\external_api {
    ...
    public static function execute($params) {
        ...
        foreach ($trainee_query as $user) {
            $cars=$DB->get_records('ticketmanagement_cars',['userid'=>$user->id]);
            $car_list=[];
            foreach ($cars as $car){
                $car_list[]=[
                    'id'=>$user->id,
                    ...
                    'color'=>$car?->color ?? '',
                    'delivery_date'=>$car?->delivery_date ?? 0,
                    'refund_date'=>$car?->refund_date ?? 0,
                ];
            };
            $total_car_list = array_merge($total_car_list, $car_list);
        }
        
        $users=[
            'car_list'=>$total_car_list,
    ...
    public static function execute_returns() {
        ...
        return new external_single_structure(
            array(
                'car_list' => new external_multiple_structure(
                    new external_single_structure([
                        ...
                        'brand'=> new external_value(PARAM_TEXT, 'brand'),
                        'model'=> new external_value(PARAM_TEXT, 'model'),
                        'color'=> new external_value(PARAM_TEXT, 'color'),
                        'platenumber'=> new external_value(PARAM_TEXT, 'Plate number'),
                        'delivery_date'=> new external_value(PARAM_INT, 'date '),
                        'refund_date'=> new external_value(PARAM_INT, 'date '),
