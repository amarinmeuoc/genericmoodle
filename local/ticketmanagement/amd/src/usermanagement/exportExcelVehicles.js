...
export const init=(XLSX, filesaver,blobutil)=>{
    ...
    const boexport=document.querySelector('#id_exportExcelVehicles');
...
const prepareDataToSend=(obj, url,token)=>{
    ...
    const service= 'local_ticketmanagement_get_list_cars_excel';
...
const onLoadFunction=(myXhr)=>{
    ...
    if (myXhr.readyState===4 && myXhr.status===200){
        ...
        createExcelFromJSON(res,'vehiclesReport');
...
const createExcelFromJSON = (res, op) => {
    ...
    if (res.car_list && res.car_list.length>0){
        ...
        const usersArray = res.car_list.map(user => [
            ...
            user.color,
            user.platenumber,
            formatUnixToDateTime(user.delivery_date),
            formatUnixToDateTime(user.refund_date),
        ]);
    ...
    // Añadir una hoja de Excel con el nombre "VehiclesReport"
    wb.SheetNames.push("VehiclesReport");
    const ws = XLSX.utils.aoa_to_sheet(listado);
    wb.Sheets["VehiclesReport"] = ws;

    // Generar y descargar el archivo
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'binary' });
    const nameFile = `VehiclesReport-${dateFile}.${month}.${year}-${hour}.${min}.xlsx`;
